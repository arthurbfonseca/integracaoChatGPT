...
import InputTextIn from "../components/Input/InputTextIn";
import InputTextOut from "../components/Input/InputTextOut";
import Button from "../components/Button/Button";
import OpenAI from "openai";

export default function Home() {
  const [inputText, setInputText] = useState("");
  const [out, setOut] = useState("No que posso te ajudar hoje?");

  const handleSubmit = () => {
    console.log("Valor:", inputText);
  };

  return (
    <div className="flex justify-center items-center flex-col min-h-screen bg-[#0e0e1f]">
      {/* <span className=" text-2xl font-bold font-mont text-[#10a37f]">
        Título
      </span> */}
      <div className="w-1/12 pt-8">
        <svg
      ...
      <div className="flex flex-row justify-center items-center w-5/6">
        <div className="flex justify-between items-center w-full font-bold font-mont text-[#10a37f] mt-32">
          <InputTextIn
            tweet
            topLabel="Entrada"
            onTextChange={(newValue) => setInputText(newValue)}
            value={inputText}
          />
          <Button text="Enviar" onSubmit={handleSubmit} />
          <InputTextOut topLabel="Saída" label={out} />
        </div>
      </div>
    </div>
  );
